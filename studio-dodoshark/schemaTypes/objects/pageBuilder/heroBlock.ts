...
export default defineType({
  ...
  fields: [
    ...
    defineField({name: 'backgroundImage', title: 'Background Image', description: 'Used only by the split product showcase variant.', type: 'image', options: {hotspot: true}, hidden: ({parent}) => (parent as {variant?: string} | undefined)?.variant !== 'splitProductShowcase', validation: (rule) => rule.custom((value, context) => { if ((context.parent as {variant?: string} | undefined)?.variant === 'splitProductShowcase' && !value) return 'Background image is required for split product showcase.'; return true }), fields: [{name: 'alt', type: 'string', title: 'Alt Text', validation: (Rule) => Rule.required()}]}),
    ...
    defineField({name: 'alignment', title: 'Alignment', type: 'string', hidden: ({parent}) => (parent as {variant?: string} | undefined)?.variant === 'splitProductShowcase', options: {list: [{title: 'Left', value: 'left'}, {title: 'Right', value: 'right'}], layout: 'radio'}, initialValue: 'left'}),
    defineField({name: 'mediaLayout', title: 'Media Layout', type: 'string', hidden: ({parent}) => (parent as {variant?: string} | undefined)?.variant !== 'splitProductShowcase', options: {list: [{title: 'Text Left / Image Right', value: 'textLeftImageRight'}, {title: 'Image Left / Text Right', value: 'imageLeftTextRight'}], layout: 'radio'}, initialValue: 'textLeftImageRight'}),
  ],
  preview: {
    select: {
      title: 'title',
