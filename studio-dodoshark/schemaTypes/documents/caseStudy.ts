...
export default defineType({
  ...
  fields: [
    ...
    defineField({name: 'industry', title: 'Industry', type: 'reference', to: [{type: 'category'}], group: 'basic', description: 'Used in filters and metadata.'}),
  ...
  preview: {
    select: {title: 'title', location: 'location', industryTitle: 'industry.title', excerpt: 'excerpt', metrics: 'impactStats', media: 'coverImage'},
    prepare({title, location, industryTitle, excerpt, metrics, media}) {
      return {
        title: title || 'Untitled case study',
        subtitle: pickText(joinPreview([location, industryTitle, itemCount(metrics) ? `${itemCount(metrics)} impact stats` : undefined]), excerpt) || 'Case study document',
        media,
      }
    },
  },
